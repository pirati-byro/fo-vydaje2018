...
Vypreparování dat z žádostí o proplacení za rok 2017, 2018, ...
...
def make_url(no):
    return 'https://wiki.pirati.cz/fo/vydaje/fo_' + str(no) + '_' + str(YEAR)
...
def parse_request(no):
    '''Stáhni žádost o proplacení s pořadovým číslem no v roce YEAR'''
    global vydaje

    logging.info('Procesuje se žádost o proplacení FO '+str(no)+'/' + str(YEAR))
    ...
    try:
        znacka=dl.find_all('dd', class_='znacka')[0].get_text().strip()
    except Exception:
        logging.warning('Nepodařilo se najít značku')

    try:
        slozka=dl.find_all('dd', class_='slozka')[0].a.get_text().strip()
    except Exception:
        logging.warning('Nepodařilo se najít složku')
        slozka = ''

    try:
        polozka_url=dl.find_all('dd', class_='polozka')[0].a['href'].strip().split('/')
    ...
    try:
        nazev=dl.find_all('dd', class_='nazev')[0].get_text().strip()
    except Exception:
        logging.warning('Nepodařilo se najít název')
        nazev=''

    try:
        castka=dl.find_all('dd', class_='castka')[0].get_text().strip()
    except Exception:
    ...
    try:
        podano=dl.find_all('dd', class_='podano')[0].get_text().strip()
    except Exception:
    ...
    redmine_url= 'https://redmine.pirati.cz/projects/fo/issues.csv?c%5B%5D=subject&c%5B%5D=assigned_to&c%5B%5D=status&c%5B%5D=priority&f%5B%5D=subject&f%5B%5D=&group_by=&op%5Bsubject%5D=%7E&set_filter=1&t%5B%5D=&utf8=%E2%9C%93&v%5Bsubject%5D%5B%5D=FO+'+str(no)+'%2F' + str(YEAR)
